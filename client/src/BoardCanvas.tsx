...
import { SQ, FILES, RANKS } from './constants';
...
function paintSniperAnim(
  ...
  const secondaryCol = isRed ? '#ff8800' : '#65a30d';
...
function paintTransformAnim(
  ...
  const accentColor    = isUp ? '#ffa500' : '#6d28d9';
...
export const BoardCanvas = React.memo(function BoardCanvas(props: BoardCanvasProps) {
  const {
    board, turn, sel, hints, lm, drag, dragPos, check, kingPos,
    ...
    const draw = (now: number) => {
      ...
      for (let ri = 0; ri < 8; ri++) {
        for (let ci = 0; ci < 8; ci++) {
          ...
          const isLava    = lavaSquares.some((l: LavaSquare) => l.row===row && l.col===col);
          const isLavaExp = lavaExploding.some((l: Sq) => l.row===row && l.col===col);
      ...
      if (invisibleUnder) {
        const { row: ur, col: uc, piece: up, ownerColor } = invisibleUnder;
        const uri = 7 - ur;
