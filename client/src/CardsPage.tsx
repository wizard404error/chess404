...
interface CardsPageProps { onNavigate:(page:string)=>void; }
...
export default function CardsPage({onNavigate}:CardsPageProps) {
  ...
  return (
    <div style={{height:'100vh',display:'flex',flexDirection:'column',fontFamily:"'Rajdhani','Segoe UI',sans-serif",backgroundImage:'url(/background.png)',backgroundSize:'cover',backgroundPosition:'center',backgroundAttachment:'fixed',overflow:'hidden'}}>
      <style>{`
      ...
      {/* Background overlay */}
      <div style={{position:'fixed',inset:0,background:'rgba(6,3,16,0.55)',pointerEvents:'none',zIndex:0}}/>
      ...
      {/* NAV */}
      <nav style={{position:'relative',zIndex:100,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 48px',height:'64px',background:'linear-gradient(180deg,#1a1a3a 0%,#12122a 100%)',backdropFilter:'blur(20px)',borderBottom:'1px solid rgba(255,165,40,0.25)',boxShadow:'0 2px 24px rgba(0,0,0,0.5)'}}>
        ...
        <div style={{display:'flex',gap:8}}>
          {['Play','Cards','Rankings','Community','Settings'].map(label=>{
      ...
      {/* FILTER BAR */}
      <div style={{position:'relative',zIndex:99,flexShrink:0,display:'flex',alignItems:'center',gap:10,padding:'12px 48px',background:'linear-gradient(180deg,#16163a 0%,#10102e 100%)',backdropFilter:'blur(16px)',borderBottom:'1px solid rgba(255,165,40,0.15)',flexWrap:'wrap'}}>
