...
import { makeBoard, cloneBoard, findKing, isAttacked, inB, legalMoves, gameStatus, insuffMat, positionKey, threefold, toFEN, moveNotation, uciToSan, anyLegal } from './chessEngine';
...
export default function App() {
  ...
  const [reviewTurn,  setReviewTurn]  = React.useState<PieceColor>('white');
  ...
  const doMove = React.useCallback((fr: number, fc: number, tr: number, tc: number, forcePromo?: PieceType) => {
    ...
    if (t === 'white' && !blackMovedRef.current) {
      ...
      if (abortRef.current) { clearInterval(abortRef.current); abortRef.current = null; }
    ...
    setDrawOffer(null);
  }, [resetCardUsed, startAbortCountdown, setTicking, checkEndGame, handleLavaLanding]);
  ...
  }, [promo, board, turn, posHist, resetCardUsed, setTicking, checkEndGame, over]);
  ...
  const checkFusionRedundancy = React.useCallback((
    ...
    sqA: Sq,         // square of consumed piece (to check bishop color)
    sqB: Sq,         // square of surviving piece
  ): string | null => {
    ...
    switch (mechanic) {
      ...
      case 'halffuse': {
        ...
        if (step === 2) {
          ...
          const redundancy = checkFusionRedundancy(type1, piece.type, sq1, { row, col });
  ...
  }, [cardPending, board, finishCardUse, getSafeTransforms, activateDoubleMove, triggerSwapAnim, triggerTeleportAnim, triggerSacrificeAnim, triggerMindControlAnim, triggerFuseAnim, checkFusionRedundancy, isAttackedWithFusion]);
  ...
  const handlePromoPick = React.useCallback((type: PieceType) => {
    ...
    setTimeout(() => setCardMsg(''), 2000);
    finishCardUse(card, playerColor);
  }, [cardPending, promoPicker, board, finishCardUse, triggerTransformAnim]);
  ...
  }, [stop, setTicking, startAbortCountdown]);
  ...
  const dragStart = React.useCallback((e: React.DragEvent, r: number, c: number) => {
    if (cardPending || isReviewing || over || promo) { e.preventDefault(); return; }
    const p = board[r][c];
    const ghostDrag = ghostRef.current;
    const isGhostDragSq = ghostDrag && ghostDrag.ownerColor === turn && ghostDrag.row === r && ghostDrag.col === c;
    if (!isGhostDragSq && (!p || p.color !== turn)) { e.preventDefault(); return; }
    setDrag({ row: r, col: c });
    setSel({ row: r, col: c });
    setHints(getMoves(r, c));
  ...
  }, [cardPending, isReviewing, over, promo, board, turn, getMoves]);

  const drop = React.useCallback((e: React.DragEvent, r: number, c: number) => {
    e.preventDefault();
    if (!drag || isReviewing || cardPending) return;
    const mv = getMoves(drag.row, drag.col);
    if (mv.some(m => m.row === r && m.col === c)) doMove(drag.row, drag.col, r, c);
    setDrag(null); setSel(null); setHints([]);
  }, [drag, isReviewing, cardPending, getMoves, doMove]);
  ...
  const getBombRadiusHighlight = React.useCallback((row: number, col: number): boolean => {
    for (const bomb of bombPieces) {
      if (Math.abs(row - bomb.row) <= 1 && Math.abs(col - bomb.col) <= 1) return true;
    }
    return false;
  }, [bombPieces]);
  ...
  return (
    <>
    {activePage === 'Cards' && <CardsPage onNavigate={(page: string) => setActivePage(page)} />}
    {activePage !== 'Cards' && (
    <div style={{
      ...
      <nav style={{
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'2px' }}>
          {['Play','Cards','Rankings','Community','Settings'].map((label, i) => (
        ...
        <div style={{ width:'290px', flexShrink:0, display:'flex', flexDirection:'column', gap:'10px', paddingTop:'10px', paddingBottom:'10px', paddingRight:'24px' }}>
          ...
          <div style={{
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ color:'#e8d8ff', fontWeight:700, fontSize:'16px', letterSpacing:'0.3px' }}>Opponent</div>
              <div style={{ display:'flex', alignItems:'center', gap:'10px', marginTop:'5px' }}>
                <span style={{ color:'#b088f0', fontSize:'12px', fontWeight:600 }}>♟ 1300</span>
            ...
            {cardPending ? (
              <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'12px', padding:'16px' }}>
                ...
                {(cardPending.mechanic === 'halffuse' || cardPending.mechanic === 'fullfusion') && (() => {
                  const sq1   = cardPending.data?.sq1 as Sq | undefined;
          ...
          <div style={{
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ color:'#d0fce8', fontWeight:700, fontSize:'16px', letterSpacing:'0.3px' }}>Player</div>
              <div style={{ display:'flex', alignItems:'center', gap:'10px', marginTop:'5px' }}>
                <span style={{ color:'#52c77a', fontSize:'12px', fontWeight:600 }}>♟ 1280</span>
        ...
        <div style={{
          ...
          <div style={{ background:'rgba(255,140,0,0.06)', border:'1px solid rgba(255,165,40,0.18)', borderRadius:'12px', padding:'10px 14px', flexShrink:0 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr auto 1fr', gap:'5px 10px', alignItems:'center' }}>
              <div style={{ textAlign:'right', color:'rgba(255,255,255,0.35)', fontSize:'9px', fontWeight:700 }}>⚪ White · 1280</div><div /><div style={{ color:'rgba(255,255,255,0.35)', fontSize:'9px', fontWeight:700 }}>⚫ Black · 1300</div>
            ...
            {drawOffer && drawOffer !== turn && (
              <div style={{ background:'rgba(243,156,18,0.12)', border:'1px solid rgba(243,156,18,0.45)', borderRadius:'7px', padding:'8px 10px' }}>
                ...
                <div style={{ display:'flex', gap:'6px' }}>
                  <button onClick={() => { finalPositionRef.current = { fen: toFEN(board, turn, moved, lm, hmc, fmn), turn }; setOver(true); setWinner('draw'); setDrawOffer(null); }} style={{ flex:1, padding:'5px', background:'linear-gradient(180deg,#27ae60,#1e8449)', color:'#fff', border:'none', borderRadius:'5px', cursor:'pointer', fontWeight:'bold', fontSize:'12px' }}>✓ Accept</button>
                  <button onClick={() => setDrawOffer(null)} style={{ flex:1, padding:'5px', background:'linear-gradient(180deg,#c0392b,#96281b)', color:'#fff', border:'none', borderRadius:'5px', cursor:'pointer', fontWeight:'bold', fontSize:'12px' }}>✕ Decline</button>
                </div>
              </div>
            )}
              ...
              {over ? (
                ...
                <>
                  {movHist.length === 0 || (movHist.length === 1 && !movHist[0].b) ? (
                    <button onClick={() => { if (abortRef.current) { clearInterval(abortRef.current); abortRef.current = null; } setAbortActive(false); setWinner('aborted'); setOver(true); }}
                  ...
                  <button onClick={() => { finalPositionRef.current = { fen: toFEN(board, turn, moved, lm, hmc, fmn), turn }; setOver(true); setWinner(OPP[turn]); }}
                  ...
                  {!drawOffer
                    ? <button onClick={() => setDrawOffer(turn)} style={{ flex:1, padding:'9px', fontSize:'12px', background:'linear-gradient(180deg,#8a6010,#5a3e08)', color:'#fff', border:'1px solid rgba(240,160,30,0.4)', borderRadius:'7px', cursor:'pointer', fontWeight:'bold', boxShadow:'0 2px 12px rgba(180,120,20,0.4)' }}>🤝 Draw</button>
              ...
              <input
                ...
                onKeyDown={e => {
                  if (e.key === 'Enter' && chatInput.trim()) {
                    setChatMessages(prev => [...prev, { sender: turn, text: chatInput.trim() }]);
                    setChatInput('');
                  }
                }}
