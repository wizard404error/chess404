...
func main() {
	...
	srv := &http.Server{
		Addr:              addr,
		Handler:           rate_limit.NewHeaderStrippingMiddleware("X-Powered-By")(httputil.WithRecovery(httputil.WithLogging("matchmaking-service", rate_limit.SecurityHeadersMiddleware(httputil.LimitBody(rate_limit.CSRFMiddleware(rl.Middleware(rate_limit.DefaultQueueWindow, rate_limit.DefaultQueueLimit)(mux), nil, "")))))),
